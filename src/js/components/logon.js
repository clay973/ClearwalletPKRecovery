...
function LogonViewModel() {
  ...
  self.openWallet = function() {
      ...
      //generate the wallet ID from a double SHA256 hash of the passphrase
      var hashBase = CryptoJS.SHA256(self.sanitizedEnteredPassphrase());
      var hash = CryptoJS.SHA256(hashBase).toString(CryptoJS.enc.Base64);
      ...
      //CS - No need to check if wallet is online - skip self.onIsWalletOnline and goto self.onReceivedPreferences (addition)
      //See if any servers show the wallet as online (this will return the a true result, if any server shows the wallet as online)
      //multiAPI("is_wallet_online", {'wallet_id': WALLET.identifier()}, self.onIsWalletOnline);
      
      self.onReceivedPreferences(null);
  ...
  self.onReceivedPreferences = function(data) {
    ...
    if(data) { //user stored preferences located successfully
      ...
      // CS - we dont want to save preferences
      mustSavePreferencesToServer = false;
    }
  ...
  self.displayLicenseIfNecessary = function(mustSavePreferencesToServer) {
    ...
    // CS - Skip license and goto self.openWalletPt2
